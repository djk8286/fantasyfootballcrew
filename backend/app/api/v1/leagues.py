...
async def create_league(league_data: LeagueCreate, db: AsyncSession = Depends(get_db)):
    league = League(
        name=league_data.name,
        description=league_data.description,
        commissioner_id="placeholder",  # Will come from auth middleware
...
async def update_league_scoring(
    league_id: str, data: ScoringConfigUpdate, db: AsyncSession = Depends(get_db)
):
...
async def update_league(league_id: str, update_data: LeagueUpdate, db: AsyncSession = Depends(get_db)):
    """Update league settings (commissioner only — enforced in frontend for now)."""
    result = await db.execute(
        select(
            League,
            func.count(Team.id).label("team_count")
...
async def manage_commissioners(league_id: str, req: CommissionerUpdate, db: AsyncSession = Depends(get_db)):
    """Manage commissioner rights: add/remove co-commissioners, transfer ownership."""
    result = await db.execute(select(League).where(League.id == league_id))
    league = result.scalar_one_or_none()
    if not league:
        raise HTTPException(status_code=404, detail="League not found")
