...
async def _check_commish(league: League) -> None:
    """Stub: real auth middleware will check the token. For now we trust the frontend."""
    pass
...
async def create_adjustment(
    ...
    adj = ScoreAdjustment(
        ...
        amount=data.amount,
        reason=data.reason,
        created_by="commish",  # Will come from auth
    )
    db.add(adj)
    await db.commit()
...
async def review_trade(
    ...
    trade.reviewed_by = "commish"  # Will come from auth
    await db.commit()
