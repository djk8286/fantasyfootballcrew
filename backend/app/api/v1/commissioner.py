...
from app.services.team_recap_service import generate_and_save_recap
...
@router.post("/team-recap/generate")
@limiter.limit("10/hour")
async def generate_team_recap(
    request: Request,
    league_id: str,
    week: int,
    year: int,
    ...
    """Manual fallback for the Dashboard AI Summaries per-team weekly
    recap -- normally auto-generated by the scheduler once the week's
    scores are final (see leagues.py's GET .../team-recap for the open
    read every manager actually uses). Commissioner-only + same AI-usage
    budget as the Digest, since this is a real per-league LLM spend."""
    league = await _get_league(league_id, db)
    require_commissioner(league, current_user)
    _require_ai_enabled(league)
    await check_and_record_ai_usage(league_id, "team_recap_generate", db)
    recap = await generate_and_save_recap(league, week, year, current_user.id, db)
    if not recap:
        raise HTTPException(status_code=404, detail="No scored data for this week yet -- calculate scores first")
