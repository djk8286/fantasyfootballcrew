...
async def create_team(team_data: TeamCreate, db: AsyncSession = Depends(get_db)):
    team = Team(
        name=team_data.name,
        owner_id="placeholder",  # Will come from auth middleware
...
async def update_team(team_id: str, update_data: TeamUpdate, db: AsyncSession = Depends(get_db)):
    """Update team name, avatar_url, or co_owner_id."""
...
async def claim_team(team_id: str, req: ClaimTeamRequest, db: AsyncSession = Depends(get_db)):
    """Claim a CPU team as a human user."""
    result = await db.execute(select(Team).where(Team.id == team_id))
    team = result.scalar_one_or_none()
    if not team:
        raise HTTPException(status_code=404, detail="Team not found")
...
async def delete_team(team_id: str, db: AsyncSession = Depends(get_db)):
    """Delete a team (commissioner only — enforced in frontend for now)."""
    result = await db.execute(select(Team).where(Team.id == team_id))
    team = result.scalar_one_or_none()
    if not team:
        raise HTTPException(status_code=404, detail="Team not found")
...
async def bulk_add_cpu_teams(league_id: str, req: BulkAddTeamsRequest, db: AsyncSession = Depends(get_db)):
    """Fill a league's empty slots with CPU-controlled teams."""
