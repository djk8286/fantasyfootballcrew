...
def _format_scheduled_time(scheduled_for) -> str:
    """"Saturday, September 12, 2026 at 7:00 PM UTC" -- every recipient
    sees the same wall-clock string regardless of their own timezone,
    labeled explicitly as UTC rather than silently wrong for anyone not
    in the commissioner's zone. Good enough for a beta-scale league;
    per-recipient timezone conversion would need one more piece of data
    (each User's own timezone) this app doesn't collect anywhere yet.

    strftime's no-leading-zero flags (%-d/%-I) are a POSIX/glibc
    extension Windows' C runtime doesn't implement -- this needs to
    actually run on a Windows dev machine (not just Railway's Linux
    prod), so the day/hour are pulled out manually instead."""
    weekday_and_month = scheduled_for.strftime("%A, %B")
    hour_12 = scheduled_for.strftime("%I").lstrip("0") or "12"
    minute_and_period = scheduled_for.strftime(":%M %p UTC")
    return f"{weekday_and_month} {scheduled_for.day}, {scheduled_for.year} at {hour_12}{minute_and_period}"


async def send_draft_scheduled_email(to_email: str, league_name: str, scheduled_for, draft_link: str, db: AsyncSession) -> None:
    """Sent once, the moment a commissioner sets (or changes) a draft's
    scheduled time -- the actual "know before it starts" heads-up, not
    time-dependent like the reminder/live sends below."""
    when = _format_scheduled_time(scheduled_for)
    subject = f"Draft scheduled for {league_name}"
    text_body = (
        f"The draft for {league_name} is scheduled for {when}.\n\n"
        f"Make sure your team is set before then: {draft_link}"
    )
    html_body = _render_email(
        heading="Draft scheduled \U0001f4c5",
        body_lines=[
            f"The draft for <strong>{html_module.escape(league_name)}</strong> is scheduled for "
            f"<strong>{html_module.escape(when)}</strong>.",
            "It'll start automatically -- no need to be the one to click anything, just be ready.",
        ],
        cta_label="View League",
        cta_url=draft_link,
        footnote="If the button doesn't work, copy and paste this link: " + draft_link,
    )
    await _send(to_email, subject, text_body, html_body, f"Draft scheduled for '{league_name}'", draft_link, db, "draft_scheduled")


async def send_draft_reminder_email(to_email: str, league_name: str, scheduled_for, draft_link: str, db: AsyncSession) -> None:
    """Sent once, roughly an hour out (see scheduler.py's
    DRAFT_REMINDER_LEAD_MINUTES) -- a second nudge for anyone who missed
    or forgot the scheduling confirmation."""
    when = _format_scheduled_time(scheduled_for)
    subject = f"Draft starting soon for {league_name}"
    text_body = (
        f"The draft for {league_name} starts soon -- {when}.\n\n"
        f"Join here when it goes live: {draft_link}"
    )
    html_body = _render_email(
        heading="Draft starting soon ⏰",
        body_lines=[
            f"The draft for <strong>{html_module.escape(league_name)}</strong> starts soon -- "
            f"<strong>{html_module.escape(when)}</strong>.",
        ],
        cta_label="Join League",
        cta_url=draft_link,
    ...
    await _send(to_email, subject, text_body, html_body, f"Draft reminder for '{league_name}'", draft_link, db, "draft_reminder")


async def send_draft_live_email(to_email: str, league_name: str, draft_link: str, db: AsyncSession) -> None:
    """Sent the moment a draft actually starts -- auto-start hitting its
    scheduled time, or a commissioner clicking Start Draft manually.
    Same event either way from a recipient's perspective."""
    subject = f"The draft for {league_name} is live!"
    text_body = f"The draft for {league_name} just started.\n\nJoin now: {draft_link}"
    html_body = _render_email(
        heading="The draft is live! \U0001f3c8",
        body_lines=[
            f"The draft for <strong>{html_module.escape(league_name)}</strong> just started -- it's picking time.",
        ],
        cta_label="Join Draft",
        cta_url=draft_link,
        footnote="If the button doesn't work, copy and paste this link: " + draft_link,
    )
    await _send(to_email, subject, text_body, html_body, f"Draft live for '{league_name}'", draft_link, db, "draft_live")
