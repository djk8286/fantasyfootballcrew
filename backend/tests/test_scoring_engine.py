...
Stat dict keys throughout use Sleeper's real API field names (pass_yd,
pass_int, rush_yd, rec_yd, sack/int/fum_rec/safe for defense, fgm_20_29
etc. for kicking) -- see the comment above DEFAULT_SCORING in
scoring_engine.py for the full mapping and why it matters: the engine
does exact key lookups, so a stat dict using the wrong key silently
scores that stat as 0 rather than erroring.
"""
...
class TestQBScoring:
    def test_basic_qb(self, default_scoring, qb_stats):
        """300 pass yds (12) + 3 pass TDs (12) - 1 INT (-2) + 15 rush yds (1.5) + 3 (300-yd bonus) = 26.5"""
    ...
    def test_qb_with_2pt(self, default_scoring):
        ...
        stats = {"pass_yd": 200, "pass_td": 1, "pass_int": 0, "pass_2pt": 1}
        score = calculate_player_score(stats, default_scoring, "QB")
    ...
    def test_qb_negative_stats(self, default_scoring):
        ...
        stats = {"pass_yd": 150, "pass_td": 0, "pass_int": 3}
        score = calculate_player_score(stats, default_scoring, "QB")
    ...
    def test_qb_bonus_300(self, default_scoring, qb_stats):
        ...
        # qb_stats has 300 pass_yd => should trigger pass_300_yds bonus
        score = calculate_player_score(qb_stats, default_scoring, "QB")
        # 23.5 + 3 (bonus) = 26.5
        assert score == 26.5
    ...
    def test_qb_bonus_400(self, default_scoring):
        ...
        stats = {"pass_yd": 410, "pass_td": 3, "pass_int": 0}
        score = calculate_player_score(stats, default_scoring, "QB")
...
class TestRBScoring:
    ...
    def test_rb_bonus_100(self, default_scoring, rb_stats):
        ...
        # rb_stats has 100 rush_yd => should trigger rush_100_yds bonus
        score = calculate_player_score(rb_stats, default_scoring, "RB")
        # 23 + 3 (bonus) = 26
        assert score == 26.0
...
class TestWRScoring:
    ...
    def test_wr_bonus_100(self, default_scoring, wr_stats):
        ...
        # wr_stats has 120 rec_yd => trigger rec_100_yds bonus
        score = calculate_player_score(wr_stats, default_scoring, "WR")
        # 26 + 3 (bonus) = 29
        assert score == 29.0
    ...
    def test_wr_with_rush(self, default_scoring):
        ...
        stats = {"rec": 5, "rec_yd": 60, "rec_td": 0, "rush_yd": 15, "rush_td": 1}
    ...
    def test_wr_slot_ppr_merchant(self, default_scoring):
        ...
        stats = {"rec": 12, "rec_yd": 85, "rec_td": 0}
        score = calculate_player_score(stats, default_scoring, "WR")
...
class TestTEScoring:
    ...
    def test_te_elite(self, default_scoring):
        ...
        stats = {"rec": 10, "rec_yd": 150, "rec_td": 2}
...
class TestKickerScoring:
    def test_basic_kicker(self, default_scoring, kicker_stats):
        """2 FG (30-39) (6) + 1 FG (40-49) (4) + 3 XP (3) = 13"""
        score = calculate_player_score(kicker_stats, default_scoring, "K")
        assert score == 13.0

    def test_kicker_long_fg(self, default_scoring):
        """Kicker hitting a 50-59 yarder."""
        stats = {"fgm_30_39": 0, "fgm_40_49": 0, "fgm_50_59": 2, "xpm": 1}
    ...
    def test_kicker_perfect_day(self, default_scoring):
        ...
        stats = {"fgm_30_39": 2, "fgm_40_49": 2, "fgm_50_59": 1, "xpm": 4}
        score = calculate_player_score(stats, default_scoring, "K")
...
class TestDefenseScoring:
    ...
    def test_defense_shutout(self, default_scoring):
        ...
        stats = {"sack": 2, "int": 1, "fum_rec": 0, "def_td": 0, "st_td": 1}
...
class TestEdgeCases:
    ...
    def test_none_stats(self, default_scoring):
        ...
        stats = {"pass_yd": None, "pass_td": 2, "pass_int": None}
        score = calculate_player_score(stats, default_scoring, "QB")
        # 2*4 = 8
        assert score == 8.0
    ...
    def test_all_stat_types_as_strings(self, default_scoring):
        ...
        stats = {"pass_yd": "250", "pass_td": "2", "pass_int": "0"}
        score = calculate_player_score(stats, default_scoring, "QB")
    ...
    def test_missing_stat_is_zero(self, default_scoring):
        ...
        stats = {"pass_yd": 200}  # no pass_td, no pass_int
        score = calculate_player_score(stats, default_scoring, "QB")
        # 200*0.04 = 8
        assert score == 8.0
    ...
    def test_negative_yards(self, default_scoring):
        ...
        stats = {"rush_yd": -5, "rush_td": 0, "rec": 1, "rec_yd": 8}
        score = calculate_player_score(stats, default_scoring, "RB")
...
class TestBonuses:
    def test_no_bonus_below_threshold(self, default_scoring):
        ...
        stats = {"pass_yd": 280, "rush_yd": 95, "rec_yd": 90}
    ...
    def test_multiple_bonuses(self, default_scoring):
        ...
        stats = {"pass_yd": 320, "rush_yd": 110, "rec_yd": 50}
        bonus = _calculate_bonus(stats, default_scoring["bonus"])
        # pass_300_yds (3) + rush_100_yds (3) = 6
        assert bonus == 6.0

    def test_empty_bonus_rules(self):
        bonus = _calculate_bonus({"pass_yd": 400}, {})
        assert bonus == 0.0
...
class TestLongTDEstimation:
    def test_rb_long_td(self):
        ...
        stats = {"rush_yd": 150, "rush_td": 2, "rec_yd": 0, "rec_td": 0}
    ...
    def test_rb_short_td(self):
        ...
        stats = {"rush_yd": 30, "rush_td": 3, "rec_yd": 0, "rec_td": 0}
    ...
    def test_wr_deep_threat(self):
        """WR with deep TDs."""
        stats = {"rec_yd": 200, "rec_td": 2, "rush_yd": 0, "rush_td": 0}
...
class TestCustomRules:
    def test_custom_threshold(self):
        ...
        rules = [
            {"stat_name": "pass_yd", "operator": ">=", "threshold": 350, "points": 5}
        ]
        points = _calculate_custom({"pass_yd": 380}, rules)
        assert points == 5.0
    ...
    def test_custom_not_met(self):
        ...
        points = _calculate_custom({"pass_yd": 300}, rules)
        assert points == 0.0

    def test_custom_per_unit(self):
        """Custom rule: points per unit above threshold."""
        rules = [
            {"stat_name": "rush_yd", "operator": "per_unit", "threshold": 100, "points": 0.5}
        ]
        points = _calculate_custom({"rush_yd": 120}, rules)
        # 120 - 100 = 20 * 0.5 = 10
        assert points == 10.0
    ...
    def test_custom_multiple_rules(self):
        ...
        rules = [
            {"stat_name": "rush_yd", "operator": ">=", "threshold": 100, "points": 3},
            {"stat_name": "rec", "operator": ">=", "threshold": 5, "points": 2},
        ]
        points = _calculate_custom({"rush_yd": 120, "rec": 7}, rules)
        assert points == 5.0

    def test_custom_empty_rules(self):
        points = _calculate_custom({"pass_yd": 400}, [])
        assert points == 0.0
    ...
    def test_custom_lt_operator(self):
        rules = [{"stat_name": "pass_int", "operator": "<", "threshold": 1, "points": 3}]
        points = _calculate_custom({"pass_int": 0}, rules)
        assert points == 3.0
...
class TestWeeklyScoring:
    def test_full_roster_weekly(self, default_scoring, qb_stats, rb_stats, wr_stats, te_stats, kicker_stats, defense_stats):
        ...
        week_stats = {
            ...
            "player_wr": wr_stats,
            "player_te": te_stats,
            "player_k": {"fgm_30_39": 1, "fgm_40_49": 1, "fgm_50_59": 0, "xpm": 2},
            "player_def": {"sack": 2, "int": 1, "fum_rec": 0, "def_td": 0},
        }
        positions = {
            "player_qb": "QB",
            "player_rb": "RB",
    ...
    def test_player_without_stats(self, default_scoring):
        ...
        week_stats = {"player_a": {"pass_yd": 200, "pass_td": 1}}
...
class TestOptimalLineup:
    def test_basic_optimal(self, default_scoring):
        ...
        roster = {
            "qb1": {"stats": {"pass_yd": 300, "pass_td": 3}, "position": "QB", "name": "QB Elite"},
            "rb1": {"stats": {"rush_yd": 120, "rush_td": 1}, "position": "RB", "name": "RB Stud"},
            "rb2": {"stats": {"rush_yd": 80, "rush_td": 0, "rec": 3, "rec_yd": 25}, "position": "RB", "name": "RB Solid"},
            "rb3": {"stats": {"rush_yd": 15, "rush_td": 0}, "position": "RB", "name": "RB Bench"},
            "wr1": {"stats": {"rec": 8, "rec_yd": 110, "rec_td": 1}, "position": "WR", "name": "WR WR1"},
            "wr2": {"stats": {"rec": 5, "rec_yd": 70}, "position": "WR", "name": "WR WR2"},
            "wr3": {"stats": {"rec": 2, "rec_yd": 20}, "position": "WR", "name": "WR WR3"},
            "te1": {"stats": {"rec": 4, "rec_yd": 45, "rec_td": 1}, "position": "TE", "name": "TE Stud"},
            "te2": {"stats": {"rec": 1, "rec_yd": 8}, "position": "TE", "name": "TE Backup"},
            "k1": {"stats": {"fgm_30_39": 2, "xpm": 2}, "position": "K", "name": "K Kicker"},
            "def1": {"stats": {"sack": 3, "int": 2}, "position": "DEF", "name": "DST"},
        }
    ...
    def test_flex_picks_best_remaining(self, default_scoring):
        ...
        roster = {
            "rb1": {"stats": {"rush_yd": 150, "rush_td": 2}, "position": "RB", "name": "RB Elite"},
            "rb2": {"stats": {"rush_yd": 100, "rush_td": 1}, "position": "RB", "name": "RB Good"},
            "rb3": {"stats": {"rush_yd": 5, "rush_td": 0}, "position": "RB", "name": "RB Trash"},
            "wr1": {"stats": {"rec": 6, "rec_yd": 90}, "position": "WR", "name": "WR Decent"},
        }
    ...
    def test_no_k_or_def_required(self, default_scoring):
        ...
        roster = {
            ...
            "rb1": {"stats": {"rush_yd": 100, "rush_td": 1}, "position": "RB", "name": "RB1"},
            "wr1": {"stats": {"rec": 6, "rec_yd": 90, "rec_td": 1}, "position": "WR", "name": "WR1"},
        }
...
class TestIntegration:
    def test_qb_vs_rb_comparison(self, default_scoring):
        ...
        qb = {"pass_yd": 250, "pass_td": 2, "pass_int": 0}
        rb = {"rush_yd": 100, "rush_td": 1, "rec": 4, "rec_yd": 40}
    ...
    def test_full_team_projection(self, default_scoring):
        ...
        team = {
            "qb": {"pass_yd": 280, "pass_td": 2, "pass_int": 1, "rush_yd": 20},
            "rb1": {"rush_yd": 85, "rush_td": 1, "rec": 3, "rec_yd": 20},
            "rb2": {"rush_yd": 60, "rush_td": 0, "rec": 5, "rec_yd": 45},
            "wr1": {"rec": 7, "rec_yd": 100, "rec_td": 1},
            "wr2": {"rec": 4, "rec_yd": 55, "rec_td": 0},
            "te": {"rec": 3, "rec_yd": 35, "rec_td": 0},
            "flex": {"rec": 6, "rec_yd": 80, "rec_td": 0},
            "k": {"fgm_30_39": 1, "fgm_40_49": 1, "xpm": 3},
            "def": {"sack": 3, "int": 1, "def_td": 1},
        }
        positions = {
            "qb": "QB", "rb1": "RB", "rb2": "RB",
    ...
    def test_standard_ppr_rules(self, default_scoring):
        ...
        assert config["passing"]["pass_int"] == -2  # -2 for INT
