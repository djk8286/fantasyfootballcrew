...
import { leaguesApi, teamsApi, draftsApi } from "@/lib/api-client";
import {
  Trophy,
  Users,
  Calendar,
...
export default function LeagueDetailPage() {
  ...
  const handleClaimTeam = async (teamId: string) => {
    ...
    try {
      ...
      await teamsApi.claim(teamId, userId);
      // Update localStorage
      const stored = JSON.parse(localStorage.getItem("ffc_user_teams") || "{}");
      stored[id] = teamId;
            ...
            <div className="flex flex-col gap-2">
              ...
              {!isCommissioner && (
                <div className="border-t border-surface-700 pt-3 mt-1">
                  <p className="text-surface-500 text-[10px] mb-1.5">Your User ID (for claiming)</p>
                  <input
                    type="text"
                    defaultValue={getCurrentUserId()}
                    onBlur={(e) => {
                      if (e.target.value.trim()) {
                        localStorage.setItem("ffc_user_id", e.target.value.trim());
                      }
                    }}
                    placeholder="placeholder"
                    className="w-full px-3 py-1.5 bg-surface-900 border border-surface-600 rounded-lg text-white placeholder-surface-500 text-xs focus:outline-none focus:ring-1 focus:ring-gold-400"
                  />
                  <p className="text-surface-600 text-[10px] mt-1">
                    Set this to your user ID to claim CPU teams and identify your team.
                  </p>
                </div>
              )}
            </div>
